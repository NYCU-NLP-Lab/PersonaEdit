...
def compute_z(
    ...
    target_ids = tok(request["target_new"]["str"], return_tensors="pt").to(input_device)[
        "input_ids"
    ][0]
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.layer_module_tmp.format(layer):
            ...
            delta_for_layer = delta.to(layer_out.device)
            for i, idx in enumerate(lookup_idxs):

                if len(lookup_idxs)!=len(layer_out):
                    layer_out[idx, i, :] += delta_for_layer
                else:
                    layer_out[i, idx, :] += delta_for_layer
