...
def main(
    ...
    single_gpu_load: bool = False,
):
    # Set algorithm-specific variables
    if alg_name == "MEND":
        from baselines.mend import MENDHyperParams, MendRewriteExecutor

        params_class, apply_algo = MENDHyperParams, MendRewriteExecutor().apply_to_model
    else:
    ...
    if ds_path:
        current_case_dir = run_dir / Path(ds_path).stem
        current_case_dir.mkdir(parents=True, exist_ok=True)
    else:
        current_case_dir = run_dir

    print(f"Results will be stored at {current_case_dir}")
    ...
    if type(model_name) is str:
        ...
        if torch.cuda.is_available():
            ...
            for device_idx in range(torch.cuda.device_count()):
                ...
                max_memory[device_idx] = f"{int(free_mem * 0.9) // (1024 ** 2)}MiB"
            print(f"Using CUDA max_memory map: {max_memory}")
        torch_dtype = {
            "auto": "auto",
        ...
        if single_gpu_load:
            if not torch.cuda.is_available():
                raise RuntimeError("--single_gpu_load requires CUDA.")
    ...
    if ds_name == "opinionqa":
        if ds_path is None:
            raise ValueError("OpinionQA requires --ds_path or --ds_folder.")
        print(f"Loading custom OpinionQA file: {ds_path}")
        ds = ds_class(
            DATA_DIR,
            tok=tok,
            size=dataset_size_limit,
            data_rel_path=ds_path,
    ...
    if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE"]):
        ...
        del W_out
    if alg_name == "AlphaEdit":
        model_cache_name = hparams.model_name.replace("/", "_")
        cache_p_path = STATS_DIR / model_cache_name / "null_space_project.pt"
        if cache_p_path.exists():
            print(f"Load P {cache_p_path}")
            cached_p = torch.load(cache_p_path, map_location="cpu")
            if cached_p.shape == P.shape:
                P = cached_p
            else:
                print(f"Ignoring cached P with shape {cached_p.shape}; expected {P.shape}")
                cached_p = None
        if not cache_p_path.exists() or cached_p is None:
            for i, layer in enumerate(hparams.layers):
                P[i,:,:] = get_project(model,tok,layer,hparams)
            cache_p_path.parent.mkdir(parents=True, exist_ok=True)
            torch.save(P, cache_p_path)
    glue_save_location = str(current_case_dir / 'glue_eval') + '/'
    os.makedirs(glue_save_location, exist_ok=True)
    cnt = 0
    for record_chunks in chunks(ds, num_edits):
        case_result_template = str(current_case_dir / "{}_edits-case_{}.json")
        print(f"=================================================================={cnt+1}_edit==================================================================")
    ...
    delta_path = current_case_dir / "model_delta.pt"
    torch.save(deltas, delta_path)
    del model
    torch.cuda.empty_cache()
    print(f"--- Delta weights saved to {delta_path} ---")
    print("fin")
...
def normalize_opinionqa_path(path: str, opinionqa_root: Path) -> str:
    """Return a path relative to data/OpinionQA for OpinionQA dataset loading."""
    if path is None:
        return None
    ...
    try:
        return str(candidate.resolve().relative_to(opinionqa_root.resolve()))
    except ValueError:
        return path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Directory path containing multiple OpinionQA JSON files.",
    )
    parser.add_argument(
        "--use_persona",
        action="store_true",
        help="Include respondent metadata as a persona prefix for OpinionQA prompts.",
    )
    parser.add_argument(
        "--model_dtype",
        choices=["auto", "float16", "bfloat16", "float32"],
        default="auto",
        help="Torch dtype used when loading the model. bfloat16 is usually needed for single-GPU Llama editing.",
    )
    parser.add_argument(
        "--single_gpu_load",
        action="store_true",
        help="Load the whole model directly onto cuda:0 instead of using device_map=auto.",
    )
    parser.set_defaults(skip_generation_tests=False, conserve_memory=False)
    args = parser.parse_args()

    if args.ds_folder:
        target_files = sorted(glob.glob(os.path.join(args.ds_folder, "*.json")))
    else:
        target_files = [args.ds_path]

    first_run_id = None
    opinionqa_root = DATA_DIR / "OpinionQA"

    print(f"Scheduled to run {len(target_files)} task(s).")
    for i, f_path in enumerate(target_files):
        current_continue = first_run_id if i > 0 else args.continue_from_run

        current_ds_path = normalize_opinionqa_path(f_path, opinionqa_root)

        print(f"\nProcessing {i + 1}/{len(target_files)}: {current_ds_path}")
        res_run_name = main(
            args.alg_name,
            args.model_name,
            args.hparams_fname,
